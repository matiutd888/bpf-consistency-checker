...
{
	if (prog->type == BPF_PROG_TYPE_STRUCT_OPS || prog->type == BPF_PROG_TYPE_CHECKER ||
...
static int attach_checker(const struct bpf_program *prog, long cookie, struct bpf_link **link);
...
static const struct bpf_sec_def section_defs[] = {
	...
	SEC_DEF("checker+",		CHECKER, BPF_CHECKER, SEC_ATTACH_BTF, attach_checker),
};
...
#define BTF_CHECKER_PREFIX "bpf_checker_"
...
struct bpf_link *bpf_program__attach_checker(const struct bpf_program *prog)
{
...
static int attach_checker(const struct bpf_program *prog, long cookie, struct bpf_link **link)
{
	*link = bpf_program__attach_checker(prog);
	return libbpf_get_error(*link);
}
